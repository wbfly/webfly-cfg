...
// Fallbacks for other operating systems
    ...
    paths.config = paths.config || path.join(base, '.config/webfly');
    paths.data = paths.data || path.join(base, '.local/share/webfly');
    paths.cache = paths.cache || path.join(base, '.cache/webfly');
}

paths.tmp = path.resolve(path.join(tmp, 'webfly'));
